...
if max(max(last_results.Ms)) >= 18
    answer = false;
       
%Problem-specific constraints (solid-substrate fermentation):

% ds = Sin*(fsin/V)-((mu/Yxs)+(pilip/Ylips)+(picit/Ycits))*Xf-(S/V)*Fin;
elseif it.new(1) + it.new(17) + it.new(7) + it.new(13) + it.new(4) + it.new(2) + it.new(8) ... 
        + it.new(14) + it.new(11) + it.new(5) + it.new(6) == 11
    answer = false;                                        
                                                           
%dn = Nin*(fnin/V)-rhon*Xf-(N/V)*Fin;    
elseif it.new(16) + it.new(10) == 2 
    answer = false;                                        

%dxf = mu*Xf-(Xf/V)*Fin;
elseif it.new(1) + it.new(17) + it.new(7) + it.new(13) == 4
    answer = false;
%dxl = (pilip+gamma*mu)*Xf-(Xl/V)*Fin;
elseif it.new(2) + it.new(8) + it.new(14) + it.new(11) + it.new(1) + it.new(17) + it.new(7) + it.new(13) == 8
    answer = false;
%dcit = picit*Xf-(CIT/V)*Fin;
elseif it.new(3) + it.new(9) + it.new(15) + it.new(12) == 4
    answer = false;
%dqn = rhon-mu*qn;
elseif it.new(16) + it.new(10) + it.new(1) + it.new(17) + it.new(7) + it.new(13) == 6
    answer = false;
