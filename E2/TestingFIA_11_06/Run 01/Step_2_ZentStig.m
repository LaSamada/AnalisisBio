...
parfor k=1:NExp
    ...
    %Define model inputs (In this example, time and must temperature)
   
    
    %Numerical integration to obtain model dynamics (x_theta in Eq.3 in
    % Stigter & Molenaar (2015))
    [T,Xst]=ode15s(@(t,x) meta(ModelName,t,x,theta,[nx nth]),time,...
       [x0; dxdthIC(:)],options);
   
    %Save columns from 1 to nx corresponding to integrated model states
    % given the inputs
    Xstate{k}=Xst(:,1:nx);
    %Save columns posterior to column nx corresponding to integrated
    % derivative of state vector respect to parameters.
